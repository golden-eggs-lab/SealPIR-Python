...
void PIRServer::wrap_database(vector<uint8_t> &db, uint64_t ele_num, uint64_t ele_size) {
  auto db_copy(make_unique<uint8_t[]>(ele_num * ele_size));
  for (uint64_t i = 0; i < ele_num; i++) {
    for (uint64_t j = 0; j < ele_size; j++) {
      db_copy.get()[(i * ele_size) + j] = db[(i * ele_size) + j];
    }
  }
  set_database(move(db_copy), ele_num, ele_size);
}
...
PirQuery PIRServer::deserialize_query(StringStreamR &str_in) {
  PirQuery q;

  // Convert string to stringstream
  std::stringstream stream;
  stream << str_in.str_return;
  ...
  str_in.str_return = stream.str();
  // cout << "String stream complete..." << endl;

  return q;
}

int PIRServer::serialize_reply(PirReply &reply, StringStreamR &str_in) {
  // Convert string to stringstream
  std::stringstream stream;
  stream << str_in.str_return;
  ...
  // Convert stringstream to string
  str_in.str_return = stream.str();
  // cout << "String stream complete... output_size : " << output_size << endl;
