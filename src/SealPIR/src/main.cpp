...
int main(int argc, char *argv[]) {
  ...
  uint64_t number_of_items = 1 << 8; //16;
  uint64_t size_per_item = 256; //1024; // in bytes
  uint32_t N = 4096;              // Degree of the bfv polynomials - 4096

  // Recommended values: (logt, d) = (20, 2).
  uint32_t logt = 20;         // Plaintext modulus
  uint32_t d = 2;             // Recursion level
  ...
  cout << number_of_items * size_per_item << endl;
  // uint8_t db_test[number_of_items * size_per_item]; // breaks on creation of array if size too big (seg fault)
  vector<uint8_t> db_test(number_of_items * size_per_item, 0);

  // Copy of the database. We use this at the end to make sure we retrieved
  // the correct element.
  auto db_copy(make_unique<uint8_t[]>(number_of_items * size_per_item));

  cout << "Entering for loop to create db..." << endl;
  ...
  for (uint64_t i = 0; i < number_of_items; i++) {
    for (uint64_t j = 0; j < size_per_item; j++) {
      ...
      db_test[(i * size_per_item) + j] = val;
    }
  }
  // uint8_t* db_test_ptr = (uint8_t *)&db_test;

  cout << "Calling: wrap_database..." << endl;

  // Measure database setup
  auto time_pre_s = high_resolution_clock::now();
  server.wrap_database(db_test, number_of_items, size_per_item);
  ...
  StringStreamR client_stream;
  StringStreamR server_stream;

  auto time_s_query_s = high_resolution_clock::now();
  int output_size = client.generate_serialized_query(index, client_stream);
  ...
  // Deserialize reply
  PirReply reply2 = client.deserialize_reply(server_stream);
  cout << "Reply2 size: " << reply2.size() << endl;
