...
public:
  ...
  // Wrapper fxn for set_database (avoid unique_ptr type)
  void wrap_database(std::vector<std::uint8_t> &db, std::uint64_t ele_num, 
  ...
  PirQuery deserialize_query(StringStreamR &str_in);
  ...
  // bytes written
  int serialize_reply(PirReply &reply, StringStreamR &str_in);
