...
PirReply PIRClient::deserialize_reply(StringStreamR &str_in) {
  PirReply r;
  ...
  for (uint32_t i = 0; i < pir_params_.d; i++) {
    ...
    // cout << "First loop" << unsigned(i) << endl;
    
    for (uint32_t j = 0; j < (ctx_per_dimension + 1); j++) {
      // cout << "Second loop" << unsigned(j) << endl;
      Ciphertext c;
